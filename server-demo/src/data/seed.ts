...
function historialSeed(p: Partial<Pedido> & { modalidad?: string }) {
  ...
  const SEC_ESTADO = ["autorizacion_pendiente", "solicitado", "traslado_solicitado", "en_proceso", "realizado", "traslado_retorno", "completado"];
  let camino;
  if (p.estado === "cancelado") {
    camino = [arrancaAuth ? "autorizacion_pendiente" : "solicitado", "cancelado"];
  } else {
    camino = SEC_ESTADO.filter((e) => (e !== "autorizacion_pendiente" || arrancaAuth) && (e !== "traslado_solicitado" || requiereT) && (e !== "traslado_retorno" || requiereT) && (e !== "completado" || requiereT));
    const corte = camino.indexOf(p.estado || '');
    camino = corte >= 0 ? camino.slice(0, corte + 1) : [p.estado];
  }
  const actor: Record<string, string | null> = { autorizacion_pendiente: p.creadoPor ?? null, solicitado: arrancaAuth ? "u5" : (p.creadoPor ?? null), traslado_solicitado: "u3", en_proceso: "u4", realizado: "u4", traslado_retorno: "u4", completado: "u4", cancelado: p.creadoPor ?? null };
