...
export function Header({
  usuarios, role, setRole, currentUser, setCurrentUserId, service, setService, hasPermission, setPantalla
}: HeaderProps) {
      ...
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          <div className="flex rounded-xl bg-slate-200/70 p-1">
            ...
            {hasPermission("ver_servicio") && <button className={seg(role === "clinical")} onClick={() => setRole("clinical")}><Stethoscope size={15} /> Sector</button>}
          ...
          {/* Selector de usuario */}
          <div className="relative">
            <UserCircle size={16} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2" style={{ color: (ROLES as any)[currentUser.rol]?.color }} />
            <select value={currentUser.id} onChange={(e) => setCurrentUserId(e.target.value)} className="appearance-none rounded-lg border border-slate-200 bg-white py-2 pl-8 pr-8 text-sm font-medium text-slate-700 outline-none focus:border-blue-400">
              {usuarios.map((u) => <option key={u.id} value={u.id}>{u.nombre} · {(ROLES as any)[u.rol]?.label}</option>)}
            </select>
            <ChevronDown size={15} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-slate-400" />
          </div>
        </div>
